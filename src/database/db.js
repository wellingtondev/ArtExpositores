// Importar a dependencia do SQLITE 3
const sqlite3 = require("sqlite3").verbose()

// Criar o objeto que irá fazer operações banco de dados
const db = new sqlite3.Database("./src/database/database.db") // new Inicia um novo objeto

// vou exportar o objeto db
module.exports = db

// Utilizar o objeto de banco de dados, para nossas operações
/* db.serialize(() => { */

    // com commands SQL eu vou:

    // 1- Criar uma tabela com commandos SQL ( IF NOT EXISTS) -> Se não existir.           AUTOINCREMENT -> o Campo vai se auto incrementar 
/*     db.run(`
        CREATE TABLE IF NOT EXISTS places (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            image TEXT,
            name TEXT,
            address TEXT,
            address2 TEXT,
            state TEXT,
            city TEXT,
            items TEXT
        );
    `)

    // 2- Inserir dados na tabela CALLBACK -> Tem um estilo de "CHAME ESSA FUNÇÃO DE VOLTA"
    const query = `
    INSERT INTO places (
        image
        ,name,
        address,
        address2,
        state,
        city,
        items
    ) VALUES (?,?,?,?,?,?,?
    );    
`

const values = [
    "https://images.unsplash.com/photo-1528323273322-d81458248d40?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1101&q=80",
    "Colectoria",
    "Guilherme Gemballa, Jardim América",
    "Número 260",
    "Santa Catarina",
    "Rio do Sul",
    "Resíduos Eletrônicos, Lâmpadas"
]

    function afterInsertData(err){
        if(err){
            return console.log(err)
        }

        console.log("Cadastrado com Sucesso")
        console.log(this) // o this dentro dessa funcao , ele referencia a resposta que o run ta trazendo para nós
    } */
    /* db.run(query, values,afterInsertData) */

    // 3- Consultar os dados da tabela * -> Significa que vai selecionar todos os campos da tabela
/*     db.all(`SELECT * FROM places`, function(err, rows){
        if(err){
            return console.log(err)
        }

        console.log("Aqui estão seus registros")
        console.log(rows)
    }) */


    // 4- Deletar um dado da tabela
/*     db.run(`DELETE FROM places WHERE id = ?`, [7], function(err){
        if(err){
            return console.log(err)
        }

        console.log("Registro deletado com sucesso!")
    })

}) */